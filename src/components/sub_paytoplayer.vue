	...
	<div class="manage-transfer">
		...
		<mu-container class="whole-screen-wrapper" style='margin-top: 0 !important;'>
			<mu-paper :z-depth="3" class="account-info">
				<mu-list>
					<mu-list-item :ripple="false" style='color: rgb(33,150,243);'>
						<mu-list-item-title @click='openDialog'>
							<mu-flex align-items="center">
								<mu-flex justify-content="center" class="info-title">直属玩家</mu-flex>
							</mu-flex>
						</mu-list-item-title>
			...
			<mu-dialog width="360" transition="slide-bottom" fullscreen :open.sync="openFullscreen" scrollable>
				...
				<mu-paper :z-depth="3">
					<mu-list>
						<mu-list-item v-for='item in listInfo.players' :key='item.uniqueid'>
							<mu-list-item-action>
								<mu-radio :value="item.uuid" v-model="choseRadio" :label="item.nick_name" @click='send'></mu-radio>
							</mu-list-item-action>
						</mu-list-item>
					</mu-list>
				</mu-paper>
	...
	export default {
		...
		methods: {
			load() {
				...
				this.$axios.post('api/player/getList', {
			...
			withdraw() {
				...
				if(_that.withdrawAccount <= 1) {
					eventBus.$emit('showNotification', '提现金额需大于1元！')
					return
				}
				if(!_that.choseRadio || !_that.choseLabel) {
					eventBus.$emit('showNotification', '必须选择充值对象!')
					return
				}
				_that.$confirm('是否确认充值信息?充值玩家：' + _that.choseLabel, '充值确认', {
						type: 'warning',
					})
					.then(({
						result
						...
						if(result) {
							_that.$axios.post('api/recharge/addPlayerMoney', {
									token: _that.user.token,
									uuid: _that.choseRadio,
									ptUser: _that.pt_user,
								...
								.then((res) => {
									if(res.data && res.data.status == 0) {
										_that.getMoney();
										eventBus.$emit('showNotification', '充值成功');
										_that.$router.push({
											path: '/refresh/',
											query: {
												stats: '2'
